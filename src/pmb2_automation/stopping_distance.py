...
class Automation:
    ...
    def set_model_state(self, p_x, p_y, p_z, o_x, o_y, o_z, o_w):  # add rest of arguments
        self.model_state.model_name = "pmb2"  # todo: update to be dynamic
        ...
        self.model_state.pose.orientation.z = o_z  # perpendicular to wall in corridor
    ...
    def pub_model_state(self):  # set default pmb2 pose in corridoor
        time.sleep(2)
        self.set_model_state(self.model_state.pose.position.x, self.model_state.pose.position.y,
                             self.model_state.pose.position.z,
                             self.model_state.pose.orientation.x, self.model_state.pose.orientation.y,
                             self.model_state.pose.orientation.z,
    ...
    def execute_single_test_script(self, x, z, direction):
        ...
        if direction == "linear":
            ...
            while True:
                # if robot has reached target velocity
                if round(self.robot_odometry.twist.twist.linear.x, 2) >= round(self.target_velocity.linear.x, 1):
                    ...
                    # if not at target velocity, send velocity command to robot for this test case
                    self.pub_target_velocity()
                # if odometry shows robot has stopped
                if self.robot_odometry.twist.twist.linear.x <= 0 and not self.movement:
                    if not self.sd2:
        ...
        if direction == "angular":
            while True:
                if (round(self.robot_odometry.twist.twist.angular.z, 2) >= round(self.target_velocity.angular.z,
                                                                                 1)):  # if robot has reached target velocity
                    self.set_target_velocity(0.0, 0.0)
                    self.pub_target_velocity()
                    if not self.sd1:
                        self.calc_stop_dist(1)  # starting calculating stop distance
                    ...
                    self.pub_target_velocity()
                # if odometry shows robot has stopped
                if self.robot_odometry.twist.twist.angular.z <= 0 and not self.movement:
                    if not self.sd2:
                        self.calc_stop_dist(2)  # stop calculating stop distance
                    self.calc_elapsed_time(2)  # stop calculating stop distance time
                    self.calc_elapsed_time(3)  # calculate time taken to stop robot after receiving zero velocity
                    self.calc_stop_dist(3)  # calculate actual stop distance
                    # print("(IN 3) 2 dp odom=",round(self.robot_odometry.twist.twist.linear.x,2))
                    break
    ...
    def pub_data(self):  # this function publishes data for output
        ...
        self.data.initial_twist_z = 1.0  # todo: make dynamic!

        self.data.stop_distance_x = self.stop_dist_actual.position.x
        self.data.stop_distance_z = self.stop_dist_actual.orientation.z
